...
void Camera::dragMouse( int x, int y )
{
	...
	if (fmod((double)mElevation, 2.0 * M_PI) < 3 * M_PI / 2 && fmod((double)mElevation, 2.0 * M_PI) > M_PI / 2)
	{
		mUpVector[0] = -mUpVector[0];
		mUpVector[1] = -mUpVector[1];
	...
	case kActionRotate:
		{
			...
			int reflection_up_para_go = 0;
			int reflection_up_para_back = 0;
			/*
			if (!(fmod((double)mElevation, 2.0 * M_PI) < 3 * M_PI / 2 && fmod((double)mElevation, 2.0 * M_PI) > M_PI / 2))
				reflection_up_para_go++;
			if (fmod((double)mElevation, 2.0 * M_PI) < 3 * M_PI / 2 && fmod((double)mElevation, 2.0 * M_PI) > M_PI / 2)
				reflection_up_para_back++;
			*/
			...
			if (!(fmod((double)mElevation, 2.0 * M_PI) < 3 * M_PI / 2 && fmod((double)mElevation, 2.0 * M_PI) > M_PI / 2))
				reflection_up_para_back++;
			if(reflection_up_para_go == 2 || reflection_up_para_back == 2)
				mUpVector[1]= -mUpVector[1];
			*/
			break;
		}
	case kActionZoom:
		{
			...
			float anlge_up = mouseDelta[0] * M_PI / 180;
			
			mUpVector[0] = mUpVector[0] * cos(anlge_up) - mUpVector[1] * sin(anlge_up);
			mUpVector[1] = mUpVector[0] * sin(anlge_up) + mUpVector[1] * cos(anlge_up);
...
void Camera::lookAt(Vec3f eye, Vec3f at, Vec3f up) {
	...
	Vec3f dir = -eye + at;
	float tan_1 = dir[0] / dir[2];
	int angle_1 = atan();
	glRotatef();
	glMultMatrixf();
	*/
	// operater ^ : cross product
	// operater normalize()
	Vec3f F = at - eye;
	F.normalize();		//F
	up.normalize();
